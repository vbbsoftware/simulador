...
public class UncminTest_f77 extends Object implements Uncmin_methods {
	...
	public static void main(String args[]) {
		...
		while (another == 1) {
			...
			/*
			 * 
			 * Console is a public domain class described in Cornell and
			 * Horstmann's Core Java (SunSoft Press, Prentice-Hall).
			 */
			...
			UncminTest_f77 uncmintest = new UncminTest_f77(idtemp, ctemp,
					dtemp, etemp);
			...
			Uncmin_f77
					.dfault_f77(n, x0, typsiz, fscale, method, iexp, msg,
							ndigit, itnlim, iagflg, iahflg, dlt, gradtl,
							stepmx, steptl);
			iagflg[1] = 1;
			iahflg[1] = 0;
			iexp[1] = 0;
			Uncmin_f77.optif9_f77(n, x0, uncmintest, typsiz, fscale, method,
					iexp, msg, ndigit, itnlim, iagflg, iahflg, dlt, gradtl,
					stepmx, steptl, x, f, g, info, a, udiag);
	...
	public double f_to_minimize(double x[]) {
		...
		if (id_f_to_min == 0) {
			...
			f = 100.0 * (x1 * x1 - x2) * (x1 * x1 - x2) + (1.0 - x1)
					* (1.0 - x1) + 90.0 * (x3 * x3 - x4) * (x3 * x3 - x4)
					+ (1.0 - x3) * (1.0 - x3) + 10.1
					* ((1.0 - x2) * (1.0 - x2) + (1.0 - x4) * (1.0 - x4))
					+ 19.8 * (1.0 - x2) * (1.0 - x4);
	...
	public void gradient(double x[], double g[]) {
		...
		if (id_f_to_min == 0) {
			...
			g[2] = -200.0 * (x1 * x1 - x2) - 20.2 * (1.0 - x2) - 19.8
					* (1.0 - x4);
			g[3] = 360.0 * x3 * (x3 * x3 - x4) - 2.0 * (1.0 - x3);
			g[4] = -180.0 * (x3 * x3 - x4) - 20.2 * (1.0 - x4) - 19.8
					* (1.0 - x2);
	...
	public void hessian(double x[], double h[][]) {
		...
		if (id_f_to_min == 0) {
			...
			h[1][1] = 2.0 * f1d1 * f1d1 + 2.0 * f1 * f1d11 + 2.0 * f2d1 * f2d1
					+ 2.0 * f2 * f2d11;

			// h[1][2] = 2.0*f1d1*f1d2 + 2.0*f1*f1d12 +
			// 2.0*f2d1*f2d2 + 2.0*f2*f2d12;

			h[2][1] = 2.0 * f1d1 * f1d2 + 2.0 * f1 * f1d12 + 2.0 * f2d1 * f2d2
					+ 2.0 * f2 * f2d12;

			h[2][2] = 2.0 * f1d2 * f1d2 + 2.0 * f1 * f1d22 + 2.0 * f2d2 * f2d2
					+ 2.0 * f2 * f2d22;
			...
			thd11 = -one_d_twopi * fac * fac * 2.0 * x2 * x2 * x2
					/ (x1 * x1 * x1 * x1 * x1) + one_d_twopi * fac * 2.0 * x2
					/ (x1 * x1 * x1);
			thd12 = one_d_twopi * fac * fac * 2.0 * x2 * x2
					/ (x1 * x1 * x1 * x1) - one_d_twopi * fac / (x1 * x1);
			...
			h[1][1] = 2.0 * f1d1 * f1d1 + 2.0 * f1 * f1d11 + 2.0 * f2d1 * f2d1
					+ 2.0 * f2 * f2d11 + 2.0 * f3d1 * f3d1 + 2.0 * f3 * f3d11;
			...
			h[2][1] = 2.0 * f1d1 * f1d2 + 2.0 * f1 * f1d12 + 2.0 * f2d1 * f2d2
					+ 2.0 * f2 * f2d12 + 2.0 * f3d1 * f3d2 + 2.0 * f3 * f3d12;

			h[2][2] = 2.0 * f1d2 * f1d2 + 2.0 * f1 * f1d22 + 2.0 * f2d2 * f2d2
					+ 2.0 * f2 * f2d22 + 2.0 * f3d2 * f3d2 + 2.0 * f3 * f3d22;

			// h[2][3] = 2.0*f1d2*f1d3 + 2.0*f1*f1d23 +
			// 2.0*f2d2*f2d3 + 2.0*f2*f2d23 +
			// 2.0*f3d2*f3d3 + 2.0*f3*f3d23;

			h[3][1] = 2.0 * f1d1 * f1d3 + 2.0 * f1 * f1d13 + 2.0 * f2d1 * f2d3
					+ 2.0 * f2 * f2d13 + 2.0 * f3d1 * f3d3 + 2.0 * f3 * f3d13;

			h[3][2] = 2.0 * f1d2 * f1d3 + 2.0 * f1 * f1d23 + 2.0 * f2d2 * f2d3
					+ 2.0 * f2 * f2d23 + 2.0 * f3d2 * f3d3 + 2.0 * f3 * f3d23;

			h[3][3] = 2.0 * f1d3 * f1d3 + 2.0 * f1 * f1d33 + 2.0 * f2d3 * f2d3
					+ 2.0 * f2 * f2d33 + 2.0 * f3d3 * f3d3 + 2.0 * f3 * f3d33;
