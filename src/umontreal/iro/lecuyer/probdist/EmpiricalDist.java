...
/**
 * Extends {@link DiscreteDistribution} to an <SPAN
 * CLASS="textit">empirical</SPAN> distribution function, based on the
 * observations <SPAN CLASS="MATH"><I>X</I><SUB>(1)</SUB>,...,
 * <I>X</I><SUB>(n)</SUB></SPAN> (sorted by increasing order). The distribution
 * is uniform over the <SPAN CLASS="MATH"><I>n</I></SPAN> observations, so the
 * distribution function has a jump of <SPAN CLASS="MATH">1/<I>n</I></SPAN> at
 * each of the <SPAN CLASS="MATH"><I>n</I></SPAN> observations.
 * 
 */
public class EmpiricalDist extends DiscreteDistribution {
	...
	/**
	 * Constructs a new empirical distribution using all the observations stored
	 * in <TT>obs</TT>, and which are assumed to have been sorted in increasing
	 * numerical order. <A NAME="tex2html1" HREF="#foot62"><SUP><SPAN
	 * CLASS="arabic">1</SPAN></SUP></A> These observations are copied into an
	 * internal array.
	 * 
	 */
	...
	/**
	 * Constructs a new empirical distribution using the observations read from
	 * the reader <TT>in</TT>. This constructor will read the first
	 * <TT>double</TT> of each line in the stream. Any line that does not start
	 * with a <TT>+</TT>, <TT>-</TT>, or a decimal digit, is ignored. One must
	 * be careful about lines starting with a blank. This format is the same as
	 * in UNURAN. The observations read are assumed to have been sorted in
		...
		while ((li = inb.readLine()) != null) {
			...
			while (index < li.length()
					&& (li.charAt(index) == '+' || li.charAt(index) == '-'
							|| li.charAt(index) == 'e'
							|| li.charAt(index) == 'E'
							|| li.charAt(index) == '.' || Character.isDigit(li
							.charAt(index))))
				++index;
	...
	/**
	 * Returns the median. Returns the <SPAN
	 * CLASS="MATH"><I>n</I>/2<SUP>th</SUP></SPAN> item of the sorted
	 * observations when the number of items is odd, and the mean of the <SPAN
	 * CLASS="MATH"><I>n</I>/2<SUP>th</SUP></SPAN> and the
	 * 
	 * <SPAN CLASS="MATH">(<I>n</I>/2 + 1)<SUP>th</SUP></SPAN> items when the
	...
	/**
	 * Returns the median. Returns the <SPAN
	 * CLASS="MATH"><I>n</I>/2<SUP>th</SUP></SPAN> item of the array
	 * <TT>obs</TT> when the number of items is odd, and the mean of the <SPAN
	 * CLASS="MATH"><I>n</I>/2<SUP>th</SUP></SPAN> and the
	 * 
	 * <SPAN CLASS="MATH">(<I>n</I>/2 + 1)<SUP>th</SUP></SPAN> items when the
	 * number of items is even. The array does not have to be sorted.
	 * 
	 * @param obs
	...
	public static double getMedian(double obs[], int n) {
		if ((n % 2) == 0)
			return ((Misc.quickSelect(obs, n, (n / 2 - 1)) + Misc.quickSelect(
					obs, n, (n / 2))) / 2.0);
		else
	 ...
	 * for <SPAN CLASS="MATH"><I>i</I> = 0, 1,&#8230;, <I>n</I> - 1</SPAN>.
	 * 
	 */
	...
	/**
	 * Returns the <SPAN CLASS="textit">interquartile range</SPAN> of the
	 * observations, defined as the difference between the third and first
	 * quartiles.
	 * 
	 */
